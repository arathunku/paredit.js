...
describe('paredit navigator', function() {
  ...
  describe("sexp boundaries", function() {
    ...
    describe("shrink", function() {
      ...
      it(".*(xxx)*..->..(*xxx*)..", function() {
        // expect(nav.sexpRangeShrink(parse(" (aaa) "), 1,6)).deep.eq([2,5]);
      });
      ...
      it('."*xx*x")..->.."**xxx"..', function() {
        // expect(nav.sexpRangeShrink(parse(' "aaa" '), 2,4)).deep.eq([2,2]);
      });

      it("(*x* x)->(**x xx)", function() {
        expect(nav.sexpRangeShrink(parse("(a aa)"), 1,2)).deep.eq([1,1]);
      });

      it('("*x*" x)->("**x" xx)', function() {
        // expect(nav.sexpRangeShrink(parse("(a aa)"), 2,3)).deep.eq([2,2]);
      });

      it("(*x xx*)->(*x *xx)", function() {
        // expect(nav.sexpRangeShrink(parse("(a aa)"), 1,5)).deep.eq([1,3]);
      });

      it("@*@xxx*->@*@*xxx", function() {
        // expect(nav.sexpRangeShrink(parse("@@aaa"), 1,5)).deep.eq([1,2]);
      });

      it("*@@xxx*->*@@*xxx", function() {
        // expect(nav.sexpRangeShrink(parse("@@aaa"), 0,5)).deep.eq([0,2]);
      });
