...
;(function(run) {
  ...
  var nav = exports.navigator = {
    ...
    sexpRangeShrink: function(ast, startIdx, endIdx) {
      ...
      if (startIdx !== endIdx) {
        var directMatchedStart = last(w.sexpsAt(ast, startIdx, function(n) {
          return n.start === startIdx; }));
        var directMatchedEnd = directMatchedStart && last(w.sexpsAt(ast, endIdx - 1, function(n) {
          return n.end === endIdx; }));

        console.log("directMatchedStart", directMatchedStart)
        console.log("directMatchedEnd", directMatchedEnd)
        if (directMatchedStart && directMatchedEnd) {
          if(directMatchedEnd == directMatchedStart) {
            // if(directMatchedEnd.type == 'string') {
              return [directMatchedStart.start + 1, directMatchedEnd.end - 1];
            // } else {
              // return [directMatchedStart.start, directMatchedEnd.end - 1];
            // }
          }

          var directRight = last(w.sexpsAt(ast, endIdx, function(n) {
            console.log("-------> sexpsAt", n.start, n.end, endIdx);
            return endIdx >= n.end && !w.hasChildren(n); }));

          console.log("->", directRight);
          if (directRight) return [startIdx, directRight.start];

          var directLeft = last(w.sexpsAt(ast, startIdx, function(n) {
            console.log("<------ sexpsAt", n.start, n.end, startIdx);
            return n.start >= startIdx && !w.hasChildren(n); }));

          console.log("<-", directLeft)
          if (directLeft) return [directLeft.end, endIdx];
        }
      }
      ...
      if (sexp.type === 'list' || sexp.type === 'string') {
        ...
        if (sexp.start+1 >= startIdx || endIdx >= sexp.end-1) {
          console.log("NOT A BORDER SELECTION");
          return [sexp.start+1, sexp.start+1];
        }
      }
      console.log("DONE");
      return [sexp.start, sexp.end];
